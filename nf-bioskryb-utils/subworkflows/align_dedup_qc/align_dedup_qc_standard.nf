...
include { PICARD_METRICS_WF as PICARD_METRICS_WITH_DEDUP_WF } from '../../modules/picard/metrics/main.nf'
include { PICARD_METRICS_WF as PICARD_METRICS_WITH_NONDEDUP_WF } from '../../modules/picard/metrics/main.nf'
...
workflow ALIGN_DEDUP_QC_STANDARD {
    ...
    main:
        ...
        PICARD_METRICS_WITH_NONDEDUP_WF (
        ...
        ch_versions = ch_versions.mix(PICARD_METRICS_WITH_DEDUP_WF.out.version.first())

    emit:
        nondedup_bam = BWA_WF.out.bam
        dedup_bam = PICARD_MARKDUPLICATES_WF.out.bam
        dedup_metrics = PICARD_MARKDUPLICATES_WF.out.metrics
        metrics_with_dedup = PICARD_METRICS_WITH_DEDUP_WF.out.metrics_tuple
        metrics_with_nondedup = PICARD_METRICS_WITH_NONDEDUP_WF.out.metrics_tuple
        version = ch_versions
}
